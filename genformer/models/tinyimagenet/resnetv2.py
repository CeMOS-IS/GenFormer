...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks,in_channels:int=None, num_classes:int=None, zero_init_residual=False, cfg:dict=None, **kwargs):
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), "tinyimagenet_teachers", "ResNet18", "student_best"))["model"])
        except:
            raise Exception(f"No suitable pretrained model for teacher: {model.__class__.__name__}")
    return model
...
def ResNet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), "tinyimagenet_teachers", "ResNet34", "student_best"))["model"])
        except:
            raise Exception(f"No suitable pretrained model for teacher: {model.__class__.__name__}")
    return model
...
def ResNet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), "tinyimagenet_teachers", "ResNet50", "student_best"))["model"])
        except:
            raise Exception(f"No suitable pretrained model for teacher: {model.__class__.__name__}")
    return model
...
def ResNet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), "tinyimagenet_teachers", "ResNet101", "student_best"))["model"])
        except:
            raise Exception(f"No suitable pretrained model for teacher: {model.__class__.__name__}")
    return model
...
def ResNet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), f"{cfg.DATASET.TYPE.TRAIN}_models", "ResNet152", "student_best"))["model"])
        except:
...
if __name__ == "__main__":
    ...
    net = ResNet18(in_channels=in_channels, num_classes=num_classes).cuda()
    import time
