"""
Modified from the official implementation of ConViT.
https://github.com/facebookresearch/convit/blob/main/convit.py
"""
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads,  mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., eps=1e-6, act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_gpsa=True, **kwargs):
        super().__init__()
...
class ConViT(BaseTransformerModel):

    def __init__(self, in_channels:int=None, num_classes:int=None, cfg:dict=None, qkv_bias=False, qk_scale=None, hybrid_backbone=None, use_pos_embed=True, **kwargs):
        super(ConViT, self).__init__(cfg=cfg)
        self.in_channels = in_channels
        self.num_classes = num_classes
        
        self.local_up_to_layer = cfg.MODEL.CONVIT.LOCAL_LAYERS
        self.locality_strength = cfg.MODEL.CONVIT.LOCALITY_STRENGTH
        self.use_pos_embed = use_pos_embed

        if hybrid_backbone is not None:
            self.patch_embed = HybridEmbed(
                hybrid_backbone, img_size=self.img_size, in_chans=self.in_channels, embed_dim=self.hidden_dim)
        else:
            self.patch_embed = PatchEmbed(
                img_size=self.img_size, patch_size=self.patch_size, in_chans=self.in_channels, embed_dim=self.hidden_dim)
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=self.hidden_dim, num_heads=self.num_heads, mlp_ratio=self.mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=self.drop_rate, attn_drop=self.attn_drop_rate, drop_path=dpr[i], eps=self.ln_eps, norm_layer=layernorm,
                use_gpsa=True,
                locality_strength=self.locality_strength)
            if i < self.local_up_to_layer else
            Block(
                dim=self.hidden_dim, num_heads=self.num_heads, mlp_ratio=self.mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=self.drop_rate, attn_drop=self.attn_drop_rate, drop_path=dpr[i], eps=self.ln_eps, norm_layer=layernorm,
                use_gpsa=False)
        ...
        self.feature_info = [dict(num_chs=self.hidden_dim, reduction=0, module='head')]
        self.head = nn.Linear(self.hidden_dim, self.num_classes) if self.num_classes > 0 else nn.Identity()
    ...
    def _feature_hook(self, module, inputs, outputs):
        ...
        if outputs.size(1) == self.num_patches:
            x = outputs.view(outputs.size(0), feat_size, feat_size, self.hidden_dim)
        else:
...
def convit(pretrained=False, **kwargs):
    model = ConViT(**kwargs)
    cfg = kwargs["cfg"]
    ...
    if pretrained:
        try:
            model.load_state_dict(load_checkpoint(os.path.join(
                ckpt_path(), f"cifar{kwargs['num_classes']}_models", "convit", "student_best"))["model"])
        except:
...
if __name__ == "__main__":
    ...
    cfg.MODEL.TRANSFORMER.NUM_HEADS = 4
    ...
    net = convit(in_channels=in_channels, num_classes=num_classes, cfg=cfg).cuda()
    import time
